...
Proof.
  exists (P (C 3) (P (C 3) (C 3))).
  split.
  Case "left".
    unfold not. intros Hv. inversion Hv.
  Case "right".
    unfold normal_form, not. intros Hnf. 
    induction Hnf. inversion H; subst.
    inversion H3.
Qed.
(** [] *)
...
Example bool_step_prop1_false :
  ~bool_step_prop1.
Proof.
  unfold not, bool_step_prop1. intros Hstep. inversion Hstep.
Qed.
...
Proof.  
  ...
  Case "tif t1 t2 t3".
    ...
    SCase "right".
      inversion H.
      exists (tif x t2 t3). apply ST_If. assumption.
Qed.
(** [] *)
...
Theorem step_deterministic :
  deterministic step.
Proof.
  unfold deterministic. intros x y1 y2 Hxy1 Hxy2.
  generalize dependent y2.
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_If : forall t1 t1' t2 t3,
      t1 ==> t1' ->
      tif t1 t2 t3 ==> tif t1' t2 t3
  | ST_ShortCircuit : forall t1 t2 t3 t',
      (t2 = t' \/ t2 ==> t') ->
      (t3 = t' \/ t3 ==> t') ->
      tif t1 t2 t3 ==> t'
...
Proof.
  ...
  apply ST_ShortCircuit; left; reflexivity.
Qed.
(** [] *)
...
(* Yes: the ST_ShortCircuit rules only accomplish in single steps what would otherwise have taken several steps. *)
(**
...
(* Yes: ST_ShortCircuit just provides an alternate single-step reduction for a form of expression that was already reducible. *)
(**
...
(* Yes: if we took away ANY of the original three constructors from the step relation, certain expression forms would become irreducible (normal forms) without being values. *)
*)
(** [] *)
...
Proof.
  ...
  Case "multi_step".
      ...
      SCase "multi_step".
        ...
        assert (y0 = y) as eqy0. eapply step_deterministic.
        apply H0. apply H. rewrite eqy0 in *.
        apply H1.
Qed.
(** [] *)
...
Lemma multistep_congr_2 : forall t1 t2 t2',
     ...
     P t1 t2 ==>* P t1 t2'.
Proof.
  intros t1 t2 t2' Ht1 Ht2. multi_cases (induction Ht2) Case.
    Case "multi_refl". apply multi_refl.
    Case "multi_step".
...
Proof.
  ...
  induction t; intros n0 H.
    ...
    apply (multi_step step (P (C n1) (C n2)) (C (n1 + n2))).
    apply ST_PlusConstConst.
    apply multi_refl.
Qed.
(** [] *)
...
(* Proceed by induction on t, with n left generalized in the induction hypotheses.
   
   [t = C n] -- C n ==>* C n by multi_refl, more-or-less trivially
   [t = P t1 t2] -- P t1 t2 ==>* C (n1 + n2) is the goal.
     We apply our inductive hypotheses to our preexisting assumptions t1 || n1
     and t2 || n2 to get t1 ==>* C n1 and t2 ==>* C n2.  From there we apply the
     transitive property of multistep, with multistep_congr_1 and multistep_congr_2
     providing the necessary intermediate transitions for transitivity to be useful.
     This eventually yields the desired reduction P t1 t2 ==>* P (C n1) (C n2), at
     which point we apply multi_refl and ST_PlusConstConst as base cases. *)
*)
...
Proof.
  ...
  step_cases (induction Hs) Case;
    intros n Ht; inversion Ht; subst;
    apply E_Plus.
  Case "ST_PlusConstConst".
...
Proof.
  ...
  destruct Hnf as [Hms Hsnf].
  multi_cases (induction Hms) Case.
  Case "multi_refl".
    unfold step_normal_form in Hsnf. apply nf_same_as_value in Hsnf.
    inversion Hsnf; subst.
    exists n. split. reflexivity. constructor.
  Case "multi_step".
    apply IHHms in Hsnf.
    destruct Hsnf as [n [Hz Hy]].
    exists n. split. assumption.
    apply step__eval with (t':= y); assumption.
Qed.
(** [] *)
...
Proof.
  intros t n.
  split; intros H; induction H; simpl.
  induction t.
    apply E_Const.
...
Definition compiler_is_correct_statement : Prop := forall st e n,
  let p:= (s_compile e) in
    s_execute st [] p = [n] <-> (stack_multistep st) (p, []) ([], [n]).

Lemma stack_multistep_app : forall st p1 p2 s1 s2,
  (stack_multistep st) (p1, []) ([], s1) ->
  (stack_multistep st) (p2, s1) ([], s2) ->
  (stack_multistep st) (p1 ++ p2, []) ([], s2).
Proof.
  intros st p1 p2 s1 s2 H1 H2.
  apply multi_trans with (y:= (p2, s1)).
  induction p1.
  Case "nil".
    simpl.
    inversion H1; subst. apply multi_refl. inversion H.
  Case "cons".
...
(*  unfold compiler_is_correct_statement.
  intros st e. split; generalize dependent n.
  aexp_cases (induction e) Case; intros value H;
  simpl in H; simpl.
  Case "ANum".
    inversion H; subst. apply multi_R.
    apply SS_Push.
  Case "AId".
    inversion H; subst. apply multi_R.
    apply SS_Load.
  Case "APlus".
    repeat rewrite s_compile_app in H.
    remember (s_execute st [] (s_compile e1)) as s1 eqn:Hs1.
    remember (s_execute st s1 (s_compile e2)) as s2 eqn:Hs2.
    apply multi_trans with (y:= (s_compile e2 ++ [SPlus], s_execute st [] (s_compile e1))).
    rewrite H. apply SS_Plus.
  induction p.
