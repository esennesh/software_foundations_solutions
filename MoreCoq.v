...
Proof.
  ...
  destruct (beq_nat k1 k2).
    ...
    Case "beq_nat k1 k2 = false". apply f_equal.  reflexivity.
Qed.
(** [] *)
...
Theorem pair_ident : forall (X Y : Type) (p : prod X Y),
  p = (fst p, snd p).
Proof.
  intros X Y p.
  destruct p.
...
Proof.
  ...
  rewrite <- eq.
  apply f_equal.
  apply beq_nat_true.  apply k1_eq_k2.
  intros eq.
  reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  intros l2.  induction l2 as [| h2 t2].
  reflexivity.
...
Proof.
  ...
  unfold override.
  destruct (beq_nat k1 k3) eqn:ek13.
  apply beq_nat_true in ek13.  rewrite <- ek13.
  rewrite eq. reflexivity.
  destruct (beq_nat k2 k3) eqn:ek23.
...
Fixpoint forallb (X : Type) (p : X -> bool) (l : list X): bool :=
  match l with
  | nil => true
  | cons h t => andb (p h) (forallb X p t)
  end.

Fixpoint existsb (X : Type) (p: X -> bool) (l : list X): bool :=
  match l with
  | nil => false
  | cons h t => orb (p h) (existsb X p t)
  end.

Definition existsb' (X : Type) (p: X -> bool) (l : list X): bool := 
  negb (forallb X (fun (x: X) => negb (p x)) l).

Theorem existsb_implementation_independent : forall (X: Type) (p: X -> bool)
                                                    (l: list X),
  existsb X p l = existsb' X p l.
Proof.
  intros X p l.
  induction l as [| h t].
  reflexivity.
