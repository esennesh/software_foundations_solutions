...
Lemma negb_invertable : forall b b',
  negb b = b' ->
  b = negb b'.
Proof.
  intros.
  destruct b; simpl in H; rewrite <- H; reflexivity.
Qed.
...
Proof.
  intros b e1 e2 st1 st2.  split; intros.
  Case "->".
    inversion H; subst.
    apply E_IfFalse.
  ...
  Case "<-".
    inversion H; subst.
    apply E_IfFalse.
    simpl in H5. apply negb_invertable in H5.  simpl in H5.  apply H5.
    apply H6.
    apply E_IfTrue.
    simpl in H5.  apply negb_invertable in H5.  simpl in H5.  apply H5.
    apply H6.
...
Proof.
  intros b c Hb.
  split; intros;
  apply ex_falso_quodlibet; apply WHILE_true_nonterm in H.
  apply H.  apply Hb.
  apply H.  unfold bequiv.  reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  Case "->".
    unfold aequiv in Ha.
    assert (aeval st (AId x) = aeval st e).  apply Ha.
    inversion H0.
...
Proof.
  ...
  Case "->".
    ...
    apply E_Seq with (st':= st'0);
    apply Hce1 in H1; apply Hce2 in H4;
    assumption.
  Case "<-".
    inversion Hseq; subst.
    apply E_Seq with (st':= st'0);
...
Proof.
  ...
  split; intros Hif.
  Case "->".
    inversion Hif; subst; rewrite Hbe in H4.
...
Proof. 
  ...
  Case "WHILE".
    ...
    destruct (fold_constants_bexp b) eqn:Heqb;
      ...
      SCase "bequiv b BTrue".
        apply WHILE_true.  assumption.
      SCase "bequiv b BFalse".
...
Proof.
  intros i st a ni Hunused.
  induction Hunused; simpl;
  try (rewrite IHHunused1; rewrite IHHunused2; reflexivity).
  Case "VNUNum".
    reflexivity.
  Case "VNUId".
...
Proof.
  ...
  assumption.  unfold bequiv.  reflexivity.
Qed.
(** [] *)
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st || st''
  | E_Havoc : forall (st : state) (l: id) (n: nat), (HAVOC l) / st || (update st l n)
...
Proof.
  unfold stequiv.  intros.
  induction a; try simpl; try rewrite IHa1; try rewrite IHa2; try reflexivity.
  apply H.
Qed.
(** [] *)
...
Proof.
  ...
  try replace (aeval st1 a) with (aeval st2 a);
  try replace (aeval st1 a0) with (aeval st2 a0);
  try reflexivity;
  try (symmetry; apply stequiv_aeval; assumption).
...
Proof.
  ...
  Case "->".
    ...
    assert (update (update st l1 (aeval st a1)) l2 (aeval st a2) = 
    ...
    rewrite aeval_weakening.  reflexivity.
    assumption.  assumption.
  Case "<-".
    inversion H2; subst.  inversion H5; subst.  inversion H8; subst.
    rewrite aeval_weakening.
    assert (update (update st l2 (aeval st a2)) l1 (aeval st a1) = 
            update (update st l1 (aeval st a1)) l2 (aeval st a2)).
      apply functional_extensionality.  intros.
      apply update_permute.
      unfold not in H.  unfold not.  intros.
      apply H.  symmetry in H3. assumption.
    rewrite H3.
