...
(* N, where N is the natural numbers; baz is a recursive type, so it can nest infinitely *)
[] *)
...
Lemma rev_snoc_head : forall X : Type, forall (l : list X) (h : X),
...
Theorem distr_rev : forall X (l1: list X) (l2 : list X),
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => andb (evenb n) (andb (ble_nat 7 n) (negb (beq_nat n 7)))) l.
...
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  ((filter test l), (filter (fun lst => negb (test lst)) l)).
