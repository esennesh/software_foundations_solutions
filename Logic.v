...
Proof.
  ...
  split.
  intros H.  apply IHn'.  apply H.
  intros H.  apply ev_SS.  apply IHn'.  unfold even in H.  simpl in H.
  unfold even.  apply H.
Qed.
(** [] *)
...
Proof.
  ...
  split.
  intros EP.  apply H3.  apply H.  apply EP.
  intros ER.  apply H0.  apply H4. apply ER.
Qed.
...
Theorem classic_is_excluded_middle : excluded_middle -> classic.
Proof.
  unfold classic.  unfold excluded_middle.
  intros excluded_middle P.
  assert (EM: P \/ ~P).  apply excluded_middle.
  inversion EM as [HPtrue | HPfalse].
  intros HNNP.  apply HPtrue.
  intros HNNP.  apply ex_falso_quodlibet.  apply HNNP.  apply HPfalse.
Qed.
...
Proof.
  intros n m H.
  unfold not.  intros ineq.
  induction ineq.
  rewrite <- ble_nat_refl in H.  inversion H.
  apply le_S in ineq.  apply le_ble_nat in ineq.  rewrite ineq in H.
  inversion H.
Qed.
(** [] *)
...
Proof.
  ...
  unfold not.
  intros ne x.
  assert (P x \/ ~ P x).  apply exm.
  inversion H as [HP | HnP].
  apply HP.
  apply ex_falso_quodlibet.  apply ne.
  unfold not in HnP.
  exists x.  apply HnP.
Qed.
(** [] *)
...
Proof.
  ...
  split.
  intros H.
  inversion H as [Hx H'].  inversion H' as [HxP | HxQ].
  left.  exists Hx.  apply HxP.
  right. exists Hx.  apply HxQ.
  intros Hor.  inversion Hor as [[HPx HeP] | [HQx HeQ]].
  exists HPx.  left.  apply HeP.
  exists HQx.  right. apply HeQ.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite H3 in H2.  rewrite H3.  inversion H2.
...
Theorem forallb_all {X: Type} : forall (test : X -> bool) (l: list X),
...
Theorem all_forallb {X: Type} : forall (test: X -> bool) (l: list X),
...
Theorem all_forallb_equiv {X: Type} : forall (test: X -> bool) (l: list X),
  all X (fun x => test x = true) l <-> forallb test l = true.
Proof.
  split.
  apply all_forallb.
...
Inductive in_order_merge {X: Type} : list X -> list X -> list X -> Prop := 
  | merge_first: forall l1, in_order_merge l1 [] l1
  | merge_second: forall l2, in_order_merge [] l2 l2
  | merge_cons: forall l1 l2 l3 l4 l, in_order_merge l3 l4 l -> in_order_merge (l1 ++ l3) (l2 ++ l4) ((l1 ++ l2) ++ l).

Lemma filter_map {X: Type} : forall l1 l2 (test: X -> bool),
...
Lemma all_app {X: Type} : forall (l1 l2: list X) (P: X -> Prop),
  all X P l1 -> all X P l2 -> all X P (l1 ++ l2).
Proof.
  ...
  apply IHt1.  inversion H1.  apply H4.
Qed.

Lemma app_all {X: Type} : forall l1 l2 (P: X -> Prop),
  all X P (l1 ++ l2) ->
  (all X P l1) /\ (all X P l2).
Proof.
  intros l1 l2 P H.
  split.
  induction l1 as [| h t].
    apply all_nil.
    simpl in H.
    apply all_cons.  inversion H.  apply H2.
    inversion H.  apply IHt in H3.  apply H3.

  induction l1 as [| h t].
    apply H.
    apply IHt.  inversion H.  apply H3.
Qed.

Theorem filter_all_true {X: Type} : forall l (test: X -> bool),
...
Theorem filter_all_false {X: Type} : forall l (test: X -> bool),
  all X (fun x => test x = false) l ->
  filter test l = [].
Proof.
  intros l test H.
  induction H.
...
Lemma app_eq {X: Type} : forall (l l1 l2: list X),
  l1 = l2 ->
  l ++ l1 = l ++ l2.
Proof.
  intros l l1 l2 H.
...
Theorem filter_merge {X: Type} : forall l l1 l2 (test: X -> bool),
...
Proof.
  ...
  induction Hl1.
    reflexivity.
    simpl.  rewrite H.  rewrite IHHl1.  reflexivity.
  induction Hl2.
    reflexivity.
    simpl.  rewrite H.  apply IHHl2.
  repeat rewrite filter_map.
  apply app_all in Hl1.  inversion Hl1.
  apply app_all in Hl2.  inversion Hl2.
  apply filter_all_true in H.  apply filter_all_false in H1.
  rewrite H.  rewrite H1.  rewrite app_nil_end.
  apply app_eq.  apply IHiom.
  apply H0.  apply H2.
Qed.
...
Theorem filter_challenge_2 {X: Type} : forall (l lsub: list X) (test: X -> bool),
  subseq lsub l ->
  all X (fun x => test x = true) lsub ->
  length lsub <= length (filter test l).
Proof.
  intros l lsub test Hss Hall.
  induction Hss.
  simpl.  apply O_le_n.
  simpl.  inversion Hall.  rewrite H1.  simpl.
  apply n_le_m__Sn_le_Sm.  apply O_le_n.
  simpl.  inversion Hall.  rewrite H1.  simpl.
  apply n_le_m__Sn_le_Sm.  apply IHHss.
  apply H2.
Qed.
(** [] *)
...
Definition disjoint {X: Type} (l1 l2: list X) : Prop :=
  (forall x, appears_in x l1 -> ~appears_in x l2) /\ (forall x, appears_in x l2 -> ~appears_in x l1).
...
Inductive no_repeats {X: Type} : list X -> Prop := 
  | nr_nil : no_repeats []
  | nr_cons : forall l x, ~(appears_in x l) -> no_repeats l -> no_repeats (x :: l).

Example no_repeats_fours : no_repeats [1; 2; 3; 4].
Proof.
  apply nr_cons.
  unfold not.  intros H.
...
Lemma disjoint_left {X: Type} : forall x (l1 l2: list X),
  disjoint (x :: l1) l2 -> ~appears_in x l2 -> disjoint l1 l2.
Proof.
  intros x l1 l2 Hdj Hai.
  unfold disjoint.  split.
  unfold not.  intros x0 Hail1 Hail2.
  inversion Hdj.  unfold not in H.  apply H with (x0:= x0).
  apply ai_later.  apply Hail1.  apply Hail2.

  unfold not.  intros x0 Hail2 Hail1.
  inversion Hdj.  unfold not in H.  apply H with (x0:= x0).
  apply ai_later.  apply Hail1.  apply Hail2.
Qed.

Theorem nonrepeating_disjoint {X: Type} : forall (l1 l2: list X),
  no_repeats l1 -> no_repeats l2 -> disjoint l1 l2 -> no_repeats (l1 ++ l2).
Proof.
  intros l1 l2 Hnr1 Hnr2 Hdj.
  induction l1.
    simpl.  apply Hnr2.
    simpl.  apply nr_cons.
    unfold not.  intros Hai.  apply appears_in_app in Hai.
    inversion Hai.
      inversion Hnr1.  apply H2.  apply H.
      inversion Hdj.  unfold not in H0.  apply H0 with (x0:= x).
      apply ai_here.  apply H.
    apply IHl1.  inversion Hnr1.  apply H2.
    apply disjoint_left in Hdj.  apply Hdj.
    unfold not.  inversion Hdj.
    unfold not in H.  apply H with (x0:= x).
    apply ai_here.
Qed.
(** [] *)
...
Inductive nostutter:  list nat -> Prop :=
  ...
  | ns_cons : forall m n l, nostutter (m :: l) -> m <> n -> nostutter (n :: m :: l).
...
Proof.
  ...
  inversion IHappears_in.  exists (b :: witness).
  inversion H0.  rewrite H1.  exists witness0.
  reflexivity.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | rep_repeat : forall (x: X) (l: list X), appears_in x l -> repeats (x :: l)
