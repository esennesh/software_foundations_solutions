...
Fixpoint nonzeros (l:natlist) : natlist :=
  ...
  | nil => nil
  | n :: tail =>
    let rest := nonzeros tail in
      (if beq_nat n 0 then [] else [n]) ++ rest
  end.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  ...
  | nil => nil
  | n :: tail =>
    let rest := oddmembers tail in
      if oddb n then n :: rest else rest
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  ...
  | nil => nil
  | h :: t =>
    let rest := if beq_nat v h then t else remove_one v t in
    let first:= if beq_nat v h then [] else [h] in
      first ++ rest
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h :: t => (if beq_nat v h then [] else [h]) ++ remove_all v t
  end.
...
Lemma rev_snoc_head : forall l : natlist, forall h : nat,
...
Lemma count_dist : forall l1 l2 : bag, forall n : nat,
  count n (l1 ++ l2) = count n l1 + count n l2.
Proof.
  intros l1 l2 n.
  induction l1 as [| h t].
    simpl.
