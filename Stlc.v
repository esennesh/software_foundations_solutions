...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop := 
  ...
  | s_app : forall t1 t2, substi s x (tapp t1 t2) (tapp ([x:= s] t1) ([x:= s] t2))
  | s_true : substi s x ttrue ttrue
  | s_false : substi s x tfalse tfalse
  | s_if : forall t1 t2 t3, substi s x (tif t1 t2 t3) (tif ([x:= s] t1) ([x:= s] t2) ([x:= s] t3))
.
...
Tactic Notation "substi_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "s_app"
  | Case_aux c "s_true" | Case_aux c "s_false"
  | Case_aux c "s_if" ].
...
Proof with auto.
  ...
  Case "->".
    t_cases (induction t) SCase; simpl in H;
    try (destruct (eq_id_dec x i)); subst...
  Case "<-".
    substi_cases (induction H) SCase; simpl;
    try (destruct (eq_id_dec x i));
    try (rewrite eq_id); try (rewrite neq_id); try reflexivity...
Qed.
...
Example typing_example_2_full :
  empty |-
    ...
    (TArrow TBool (TArrow (TArrow TBool TBool) TBool)).
Proof.
  repeat apply T_Abs. apply T_App with (T11:= TBool).
  apply T_Var. apply extend_eq.
  apply T_App with (T11:= TBool).
  apply T_Var. apply extend_eq.
  apply T_Var. apply extend_neq. unfold not. intros. solve by inversion.
Qed.
(** [] *)
...
Example typing_example_3 :
  exists T, 
    empty |-
      ...
      T.
Proof with auto.
  eapply ex_intro. repeat eapply T_Abs. eapply T_App. apply T_Var.
  rewrite extend_neq. rewrite extend_eq... unfold not. intros. solve by inversion.
  eapply T_App. apply T_Var. rewrite extend_neq. rewrite extend_neq. rewrite extend_eq...
  unfold not; intros; solve by inversion. unfold not; intros; solve by inversion.
  apply T_Var. rewrite extend_eq...
Qed.
(** [] *)
