...
Proof with auto.
  ...
  Case "tvar". solve by inversion 2.
  Case "tapp".
    right. inversion Ht; subst.
    destruct IHt1 with (T:= TArrow T11 T)...
    SCase "value t1".
      destruct IHt2 with (T:= T11)...
      SSCase "value t2".
        inversion H; subst; try (exists ([x0:=t2]t)); try solve by inversion...
      SSCase "t2 ==> t2'".
        inversion H0; subst. exists (tapp t1 x0)...
    SCase "t1 => t1'".
      inversion H; subst. exists (tapp x0 t2)...
  Case "tif".
    inversion Ht; subst.
    destruct IHt1 with (T:= TBool); auto; right.
    SCase "value t1".
      inversion H; subst; try inversion H3.
      SSCase "t1 = ttrue". exists t2...
      SSCase "t1 = tfalse". exists t3...
    SCase "t1 ==> t1'".
      inversion H; subst. exists (tif x0 t2 t3)...
Qed.
(** [] *)
...
Proof.
  intros t T Ht.
  unfold closed. intros x Hx. apply free_in_context with (T:= T) (Gamma:= \empty) in Hx.
...
Proof.
  ...
  Case "multi_refl".
    apply Hnot_val. apply progress in Hhas_type.
    inversion Hhas_type; subst. assumption. exfalso. auto.
  Case "multi_step".
...
Theorem types_unique : forall C t T1 T2,
  C |- t \in T1 ->
  C |- t \in T2 ->
  T1 = T2.
Proof with info_auto.
  intros C t T1 T2 HT1 HT2.
  generalize dependent T2.
  has_type_cases (induction HT1) Case; intros T2 HT2; inversion HT2; subst; auto.
  Case "T_Var". rewrite H in H2. inversion H2; subst...
  Case "T_Abs". apply IHHT1 in H4. rewrite H4...
  Case "T_App". apply IHHT1_1 in H2. inversion H2; subst...
Qed.
(** [] *)
