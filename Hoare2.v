...
Definition if_minus_plus_reloaded_com : com :=
...
Theorem if_minus_plus_reloaded_correct:
  {{ fun st => True }}
  if_minus_plus_reloaded_com
  {{ fun st => (st Y) = (st X) + (st Z) }}.
Proof.
  unfold if_minus_plus_reloaded_com.
  apply hoare_if.
  Case "Condition is true".
    eapply hoare_consequence_pre. apply hoare_asgn.
    intros st [H Hle].
    unfold assn_sub. simpl.
    unfold bassn in Hle. apply ble_nat_true in Hle. simpl in Hle.
    rewrite update_eq. repeat rewrite update_neq.
    omega.
    unfold not. intros Hfalse. inversion Hfalse.
    unfold not. intros Hfalse. inversion Hfalse.
  Case "Condition is false".
    eapply hoare_consequence_pre. apply hoare_asgn.
    intros st [H Hnle].
    unfold assn_sub. simpl.
    rewrite update_eq. repeat rewrite update_neq. reflexivity.
    unfold not. intros Hfalse. inversion Hfalse.
    unfold not. intros Hfalse. inversion Hfalse.
Qed.
...
Proof.
  ...
  Case "Loop body preserves invariant".
    ...
    intros st [HXm HB].
    unfold assn_sub. simpl.
    rewrite update_eq.  rewrite <- HXm.
    apply parity_ge_2.
    unfold bassn in HB. apply ble_nat_true in HB. simpl in HB.
    assumption.
  Case "Invariant and false guard imply postcondition".
    intros st [Hx HB].
    symmetry.
    destruct (ble_nat 2 (st X)) eqn:Hle.
    SCase "2 <= X".
      unfold bassn in HB.
      inversion Hle.
      simpl in HB.
      rewrite H0 in HB.
      unfold not in HB.
      apply ex_falso_quodlibet. apply HB.
      reflexivity.
    SCase "2 > X".
      apply ble_nat_false in Hle.
      rewrite <- Hx.
      apply parity_lt_2. assumption.
  Case "strengthening precondition to postcondition".
    intros st HXm.
    rewrite HXm. reflexivity.
Qed.
(** [] *)
