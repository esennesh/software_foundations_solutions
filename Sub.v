...
(* ANSWER: Any f that attempts to access Student-specific fields (not belonging to Person) will get stuck whenever g
           calls it with a parameter that is not of subtype Student.  Actually, if evaluation proceeds via "upcasting", 
           it will get stuck every single time as long as it accesses any fields specific to Student. *)
...
Proof with auto.
  intros U Hs.
  remember TBool as V.
  subtype_cases (induction Hs) Case; try auto; try inversion Heqv.
  Case "S_Trans". subst. assert (U = TBool). apply IHHs2...
    subst. apply IHHs1...
  Case "S_Top". inversion HeqV.
  Case "S_Arrow". inversion HeqV.
Qed.
...
Proof with eauto.
  ...
  subtype_cases (induction Hs) Case; intros V1 V2 HV.
  Case "S_Refl". exists V1. exists V2...
  Case "S_Trans".
    apply IHHs2 in HV.
    destruct HV as (U1 & U2 & HU & HV1 & HV2).
    apply IHHs1 in HU.
    destruct HU as (U3 & U4 & HS & HU1 & HU2).
...
Lemma canonical_forms_of_arrow_types : forall Gamma s T1 T2,
  ...
  exists x, exists S1, exists s2,
     s = tabs x S1 s2.
Proof with eauto.
  intros Gamma s T1 T2 HGamma Hs.
  remember (TArrow T1 T2) as T. generalize dependent T2. generalize dependent T1. 
  has_type_cases (induction HGamma) Case; intros; try (solve by inversion).
  exists x. exists T11. exists t12. reflexivity.
  rewrite HeqT in H. apply sub_inversion_arrow in H.
  destruct H as (U1 & U2 & HS & HU1 & HU2).
  apply IHHGamma in HS...
Qed.
(** [] *)
