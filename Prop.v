...
Proof.
  intros x y z.
  assert (x + (z + y) = x + z + y).
  apply plus_assoc.  rewrite H.
  assert (x + z = z + x).
  apply plus_comm.  rewrite H0.
  reflexivity.
Qed.
...
Proof.
   ...
   rewrite helper_g_times2.  rewrite plus_assoc.
...
(* The proof can't be carried out by induction on [n], because induction on [n] yields an inductive case for which we have no evidence of even-ness. *)
(** [] *)
...
Proof.
  intros n m evnm evn.
  induction evn as [| n'].
  simpl in evnm.  apply evnm.
  apply IHevn.
  inversion evnm.  apply H0.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite H'.
  rewrite plus_assoc.
  assert (p + n + p + m = (p + p) + (n + m)) as H''.
  rewrite plus_assoc.
  assert (p + n + p + m = p + (n + p) + m).
...
Inductive pal : forall (X : Type), list X -> Prop := 
  | pal_empty : forall (X: Type), pal X []
  | pal_one : forall (X: Type) (x: X), pal X [x]
  | pal_many : forall (X: Type) (x: X) (l : list X), pal X l -> pal X (x :: l ++ [x]).
...
Proof.
  ...
  simpl.
  assert (t ++ snoc (rev t) h = t ++ (rev t) ++ [h]) as H.
  rewrite snoc_app.  reflexivity.
  rewrite H.
  assert (h :: t ++ rev t ++ [h] = h :: (t ++ rev t) ++ [h]) as H'.
...
Theorem pal_eq_rev : forall (X : Type) (l : list X),
  pal X l -> l = rev l.
Proof.
  intros X l pl.
  induction pl as [| X x | X x' l'].
  reflexivity.
  reflexivity.
  simpl.
  rewrite snoc_app.  rewrite distr_rev.
...
Lemma rev_inverse {X} : forall (l m : list X),
  rev l = m -> l = rev m.
Proof.
  intros l m eq.
  assert ((rev (rev l)) = (rev m)) as H.
  apply f_equal.  apply eq.
  rewrite rev_involutive in H.
  apply H.
Qed.

Lemma app_eq_left {X} : forall (l l1 l2 : list X),
  l ++ l1 = l ++ l2 -> l1 = l2.
Proof.
  intros l l1 l2 eq.
  induction l as [| h t].
...
Lemma app_eq_right {X} : forall (l l1 l2 : list X),
  l1 ++ l = l2 ++ l -> l1 = l2.
Proof.
  intros l l1 l2 eq.
  assert (rev (l1 ++ l) = rev (l2 ++ l)) as H.
  apply f_equal.  apply eq.
  repeat rewrite distr_rev in H.
  apply app_eq_left in H.
  apply rev_inverse in H.  rewrite rev_involutive in H.
  apply H.
Qed.

Theorem palindrome_converse {X : Type} : forall (l : list X),
  l = rev l -> pal X l.
Proof.
  intros l eq.
  induction l as [| h t].
  ...
  simpl in eq.  rewrite snoc_app in eq.
  destruct (rev t) eqn:revt.
  simpl in eq.  inversion eq.
  apply pal_one.
  simpl in eq.
  inversion eq.
  apply pal_many.
  apply rev_inverse in revt.  simpl in revt.
  rewrite revt in H1. rewrite snoc_app in H1.  apply app_eq_right in H1.
  apply rev_inverse in H1.
Abort.
...
Inductive subseq {X : Type} : list X -> list X -> Prop :=
  | ss_nil : forall l, subseq nil l
  | ss_one : forall x l, subseq [x] (x :: l)
  | ss_many : forall l1 l2 x, subseq l1 l2 -> subseq (x :: l1) (x :: l2).

Theorem subseq_reflexive {X: Type}: forall (l : list X),
  subseq l l.
Proof.
  intros l.
  induction l as [| h t].
  apply ss_nil.
  apply ss_many.  apply IHt.
Qed.
...
Proof.
  ...
  apply ss_many.  apply IHeq.
  reflexivity.
Qed.

Theorem subseq_transitive {X: Type} : forall (l1 l2 l3 : list X),
  subseq l1 l2 -> subseq l2 l3 -> subseq l1 l3.
Proof.
  intros l1 l2 l3 ss1 ss2.
  generalize dependent l3.
  induction ss1.
  intros l3 ss2.  apply ss_nil.
  intros l3 ss2.
  destruct l3 as [| h3 t3].
  inversion ss2.
  inversion ss2.  apply ss_one.   apply ss_one.

  intros l3 ss2.
  destruct l3 as [| h3 t3].
  inversion ss2.  inversion ss2.
  apply ss_many.  apply IHss1.
  rewrite <- H1.  apply ss_nil.
  apply ss_many.
  apply IHss1.  apply H0.
Qed.
(** [] *)
