...
The [induction] tactic introduces the inductive hypothesis as an available fact in the proof context.
...
Theorem mult_distr : forall a b c : nat,
  a * (b + c) = (a * b) + (a * c).
Proof.
  intros a b c.
  induction a as [| a'].
  ...
  Case "a = S a'".
    simpl.
    assert (H: b + c + a' * (b + c) = a' * (b + c) + b + c).
      rewrite plus_comm.
      rewrite -> plus_assoc.
      reflexivity.
    rewrite H.
    rewrite plus_assoc.
    assert (H': b + a' * b = a' * b + b).
      rewrite plus_comm.
      reflexivity.
    rewrite H'.
    assert (H'': a' * b + b + c + a' * c = a' * b + (b + c) + a' * c).
      rewrite plus_assoc.
      reflexivity.
    assert (H3: a' * b + (b + c) + a' * c = a' * b + a' * c + (b + c)).
...
Theorem mult_id_r : forall n : nat,
  n * 1 = n.
Proof.
  intros n.
  induction n as [| n'].
...
Lemma or_b_negb_l : forall b : bool,
  orb b (negb b) = true.
Proof.
  intros b.
  destruct b.
