...
(* 1) the value of X is 3
   2) value of X is less or equal to value of Y
   3) value of X is 3 OR value of X lte value of Y
   4) Z^2 <= X AND NOT ((z+1)^2 <= X)
   5) True
   6) False *)
...
Example hoare_asgn_examples_first :
...
Example hoare_asgn_examples_second:
...
(** Informally: a = (AId X) is a counterexample, as it results in
    a paradox.  When giving X ::= X, we're supposed to change nothing,
    but that makes the post-condition version of this rule a truism
    rather than a substantial assertion.  st X = st X is true by
    reflexivity but says nothing. **)
(** [] *)
...
Example hoare_asgn_examples_2_first :
  {{ fun st => (st X) + 1 <= 5 }}
  (X ::= (APlus (AId X) (ANum 1)))
  {{ fun st => st X <= 5 }}.
Proof.
...
Example hoare_asgn_examples_2_second:
  {{ fun st => 0 <= 3 /\ 3 <= 5 }}
  (X ::= ANum 3)
  {{ fun st => 0 <= st X /\ st X <= 5 }}.
Proof.
...
Proof.
  ...
  Case "first instruction".
    ...
    unfold assert_implies.  unfold assn_sub.
    intros. simpl.
    repeat (rewrite update_eq;  rewrite update_neq).
    assumption.
    unfold not; intros; inversion H0.
    unfold not; intros; inversion H0.
    unfold not; intros; inversion H0.
...
(* The proposition cannot be proven because we don't know if X occurs free in a.  If it does, then EITHER:
   1) st X = 3, which is distinctly not given to us, OR
   2) (update st X 3) changes the value of (aeval st' a) to be something other than (aeval st a), other than n. 

   However, given the assumption that it's not, we can in fact prove the proposition. *)

Require Export Equiv.

Example hoarestate1_now_provable: forall a n,
...
Proof.
  intros.
  eapply hoare_seq.
  Case "right half of seq".
    apply hoare_asgn.
  Case "left half of seq".
    eapply hoare_consequence_pre.
    apply hoare_asgn.
    unfold assert_implies.  unfold assn_sub.
    intros. simpl.
    rewrite aeval_weakening.
    remember (update st X 3) as st' eqn:Hst'.
    rewrite update_eq.
...
Proof.
  intros.
  unfold hoare_triple. unfold hoare_triple in H.
  ...
  Case "True case".
    apply H with (st:= st).
    assumption.
    split; try unfold bassn; assumption.
  Case "False case".
    apply H0.
    split; try unfold bassn; try assumption.
    unfold not. rewrite H7.  intros. inversion H3.
Qed.

Lemma not_true_then_false : forall b : bool,
  b <> true -> b = false.
Proof.
...
Lemma negb_of_falsehood_is_true : forall b,
  negb b = false ->
  b = true.
Proof.
  intros b H. destruct b.
  Case "b = true". reflexivity.
  Case "b = false".  simpl in H.  inversion H.
Qed.  
...
Proof.
  ...
  Case "Condition is true".
    ...
    assert (st X + st Y = st Z). apply H0. rewrite H1.
    rewrite update_eq.  rewrite update_neq.  reflexivity.
    unfold not. intros. inversion H2.
  Case "Condition is false".
    unfold assert_implies.  intros.
    assert (~ bassn (BNot (BEq (AId Y) (ANum 0))) st). apply H.
    assert (st X + st Y = st Z). apply H.
    unfold bassn in H0.  simpl in H0.  apply not_true_then_false in H0.
    apply negb_of_falsehood_is_true in H0.
    apply beq_nat_true in H0.
    rewrite H0 in H1.  rewrite <- plus_n_O in H1.
    assumption.
Qed.
