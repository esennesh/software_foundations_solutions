...
Proof.
  exists (tsucc ttrue).
  unfold stuck.
  split; unfold normal_form, not; intros Contra;
  inversion Contra; inversion H; subst; inversion H1.
Qed.
(** [] *)
...
Proof.
  intros t Hval. induction Hval.
  induction H;
    unfold normal_form, not; intros Contra; inversion Contra; inversion H.
  induction H;
    unfold normal_form, not; intros Contra; inversion Contra.
    inversion H; subst.
    unfold normal_form, not in IHnvalue. apply IHnvalue.
...
Ltac succ_nf H :=
  apply nv_succ in H; eapply or_intror in H; apply value_is_nf in H;
  unfold normal_form in *; elim H.
...
Proof with eauto.
  ...
  inversion Hy2; subst; try reflexivity.
    ...
    Case "ST_IfFalse".
      inversion H3.
    Case "ST_If".
      inversion Hy1. inversion Hy1.
      rewrite (IHHy1 t1'0)...
    Case "ST_Succ".
      rewrite (IHHy1 t1'0)...
    Case "ST_PredZero".
      inversion H0.  induction H. inversion H1; subst. inversion H0; subst.
    Case "ST_PredSucc".
      succ_nf H. inversion H1; subst...
    Case "ST_Pred".
      inversion Hy1; subst.
      succ_nf H0...
      rewrite (IHHy1 t1'0)...
    Case "ST_IszeroZero".
      inversion H0. succ_nf H...
    Case "ST_Iszero".
      inversion Hy1; subst.
      succ_nf H0...
      rewrite (IHHy1 t1'0)...
...
Proof with auto.
  ...
  Case "T_Pred".
    ...
    SCase "t1 can take a step".
    inversion H as [t' H'].
    right. exists (tpred t')...
  Case "T_Iszero".
    inversion IHHT; clear IHHT.
    SCase "value t1".
    right. inversion H.
      SSCase "bvalue t1". solve by inversion 2.
      SSCase "nvalue t1". inversion H0; subst.
        exists ttrue... exists tfalse...
    SCase "t1 can take a step".
    right. inversion H as [t1' H'].
    exists (tiszero t1')...
Qed.
(** [] *)
...
(** Quick review.  Answer _true_ or _false_.  In this language...
      - Every well-typed normal form is a value. -- YES.

      - Every value is a normal form. -- YES.

      - The single-step evaluation relation is
        a partial function (i.e., it is deterministic). -- YES.

      - The single-step evaluation relation is a _total_ function. -- NO.
...
Proof with auto.
  ...
  has_type_cases (induction HT) Case; 
   ...
   Case "T_Succ". inversion HE; subst. constructor...
   Case "T_Pred". inversion HE; subst.
     SCase "t1 = tzero". constructor.
     SCase "t1 = tsucc t'". inversion HT...
     apply IHHT in H0...
...
Proof with eauto.
  intros t t' T HT HE.
  generalize dependent T.
  step_cases (induction HE) Case; intros T HT; inversion HT; subst; auto.
  inversion H1; subst...
Qed.
(** [] *)
...
Theorem normalize_ex' : exists e',
  ...
  ==>a* e'.
Proof.
  apply ex_intro with (x:= (ANum 6)). normalize.
Qed.
(** [] *)
...
Example subject_expansion_false: 
  (tif ttrue ttrue tzero) ==> ttrue ->
  |- ttrue \in TBool ->
  ~(|- (tif ttrue ttrue tzero) \in TBool).
Proof.
  intros HE HT. unfold not. intros Contra.
...
(** Suppose, that we add this new rule to the typing relation: 
   ...
   counterexample.
      - Determinism of [step] remains true (only one way to evaluate a tsucc term, despite the typing relation)

      - Progress becomes false (tsucc (tiszero tzero)) now has a type but is ill-formed

      - Preservation becomes false (see above)
...
(** Suppose, instead, that we add this new rule to the [step] relation: 
  ...
  - Determinism of [step] is now false.  (tif ttrue t2 t3) can now take two possible steps.

  - Progress remains true, because we haven't changed the fact that t2 and t3 must have identical types.
  
  - Preservation remains true, because we haven't changed the fact that t2 and t3 must have identical types.
