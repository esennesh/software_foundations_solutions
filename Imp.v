...
Proof.
  ...
  induction b;
    ...
    simpl. repeat rewrite IHb. reflexivity.
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BLe : forall a1 a2 n1 n2, aevalR a1 n1 -> aevalR a2 n2 -> bevalR (BLe a1 a2) (ble_nat n1 n2)
  | E_BNot : forall b b', bevalR b b' -> bevalR (BNot b) (negb b')
  | E_BAnd : forall be1 be2 b1 b2, bevalR be1 b1 -> bevalR be2 b2 -> bevalR (BAnd be1 be2) (andb b1 b2).
...
Proof.
  ...
  Case "<-".
    ...
    induction be; intros;
      ...
      fold beval.
      apply IHbe.  reflexivity.
      apply IHbe1.  reflexivity.
      apply IHbe2.  reflexivity.
Qed.
...
Proof.
  intros.
  unfold update.  apply neq_id.  assumption.
Qed.
(** [] *)
...
Proof.
  ...
  apply neq_id.
  rewrite <- e.  unfold not.  intros H'.  symmetry in H'.
  apply H.  apply H'.  reflexivity.
Qed.
(** [] *)
...
Theorem XtimesYinZ_spec : forall x y,
  XtimesYinZ / (update (update empty_state X x) Y y) || (update (update (update empty_state X x) Y y) Z (x * y)).
Proof.
  intros.
  unfold XtimesYinZ.
  apply E_Ass.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nw_Assign : forall id ax, no_whilesR (id ::= ax)
  | nw_Seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
  | nw_Ifb : forall ct cf bx, no_whilesR ct -> no_whilesR cf -> no_whilesR (IFB bx THEN ct ELSE cf FI).
...
Proof.
  ...
  Case "->".
  intros.
  induction c; inversion H; try constructor;
    try (apply IHc1; apply andb_true_elim1 in H1; apply H1);
    try (apply IHc2; apply andb_true_elim2 in H1; apply H1).
...
Proof.
  ...
  Case "c = ::=".
    intros.
    exists (update st id (aeval st ax)).  constructor.  reflexivity.
  Case "c = ;;".
    intros.
    assert (exists st', c1 / st || st').  apply IHno_whilesR1.  inversion H1.
    assert (exists st'', c2 / x || st'').  apply IHno_whilesR2.  inversion H3.
    exists x0.  apply E_Seq with x.  apply H2.  apply H4.
  Case "c = IFB".
    intros.
    destruct (beval st bx) eqn:eq_beval.
    assert (exists st_ct, ct / st || st_ct).  apply IHno_whilesR1.  inversion H1.
    exists x.  apply E_IfTrue.  apply eq_beval.  apply H2.
    assert (exists st_cf, cf / st || st_cf).  apply IHno_whilesR2.  inversion H1.
    exists x.  apply E_IfFalse.  apply eq_beval.  apply H2.
Qed.
(** [] *)
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | instr :: rest =>
    let stack' := match instr with
      ...
      end in s_execute st stack' rest
  end.
...
Lemma s_compile_app : forall (st: state) p1 p2 stack,
...
Lemma s_compile_correct_any_stack : forall st e stack,
  s_execute st stack (s_compile e) = (aeval st e) :: stack.
Proof.
  intros.
  generalize dependent stack.
  induction e; intros; try reflexivity;
    try simpl; try repeat rewrite s_compile_app;
...
Proof.
  intros.
  apply s_compile_correct_any_stack.
Qed.
(** [] *)
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_IfFalse : forall st st' b c1 c2 sig,
      ...
      c2 / st || sig / st' ->
      (IFB b THEN c1 ELSE c2 FI) / st || sig / st'
  | E_SeqBreak : forall c1 c2 st1 st2,
      c1 / st1 || SBreak / st2 ->
      (c1 ; c2) / st1 || SBreak / st2
  | E_SeqContinue : forall c1 c2 st1 st2 st3,
      c1 / st1  || SContinue / st2 ->
      c2 / st2  || SContinue / st3 ->
      (c1 ; c2) / st1 || SContinue / st2
  | E_WhileEnd : forall b st c,
      beval st b = false ->
      (WHILE b DO c END) / st || SContinue / st
  | E_WhileLoopContinue : forall st st' st'' b c sig,
      beval st b = true ->
      c / st || SContinue / st' ->
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  first;
  [ Case_aux c "E_Skip" | Case_aux c "E_Break" | Case_aux c "E_Assign"
  | Case_aux c "E_IfTrue" | Case_aux c "E_IfFalse"
  | Case_aux c "E_SeqBreak" | Case_aux c "E_SeqContinue"
  | Case_aux c "E_WhileEnd" | Case_aux c "E_WhileLoopContinue" | Case_aux c "E_WhileLoopBreak" ].

(** Now the following properties of your definition of [ceval]: *)

Theorem break_ignore : forall c st1 st2 s,
     (BREAK; c) / st1 || s / st2 ->
     st1 = st2.
Proof.
  intros.
...
Proof.
  intros.
  inversion H0; subst;
  apply E_WhileLoopBreak; try apply H; try apply H0.
Qed.
...
Proof.
  ...
  apply IHceval2.  apply Heqloop.  apply H0.
